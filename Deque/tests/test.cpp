...
namespace TestsByMesyarik {
    ...
    struct NotDefaultConstructible {
        ...
        NotDefaultConstructible& operator=(const NotDefaultConstructible&) =
            default;
    ...
    struct Explosive {
        int x = 0;
        Explosive(int x) : x(x) {}
        Explosive(const Explosive& /*unused*/) {
            if (x != 0) {
...
namespace TestsByUnrealf1 {
    ...
    struct Explosive {
        ...
        inline static bool exploded = false;  // NOLINT

        Explosive() : should_explode(true) {
            throw 1;
        }

        Explosive(Safeguard /*unused*/) : should_explode(false) {}

        Explosive(const Explosive& /*unused*/) : should_explode(true) {
            throw 2;
        }

        //TODO: is this ok..?
        Explosive& operator=(const Explosive& /*unused*/) {
            return *this;
        }
    ...
    struct Counted {
        ...
        Counted(const Counted& /*unused*/) : Counted() {}
    ...
    struct CheckIter {
        ...
        static_assert(std::is_same_v<std::remove_cv_t<typename traits::value_type>,
            std::remove_cv_t<T>>);
        ...
        static_assert(std::is_same_v<
            decltype(std::declval<iter>() < std::declval<iter>()), bool>);
        static_assert(
            std::is_same_v<decltype(std::declval<iter>() <= std::declval<iter>()),
            bool>);
        static_assert(std::is_same_v<
    ...
    void testWithSize() {
        ...
        assert((simple.size() == size_t(size)) &&
            std::all_of(simple.begin(), simple.end(), [](int item) {
                return item == 0;
                }));
        ...
        assert(std::all_of(default_constructor.begin(), default_constructor.end(),
            [](const auto& item) {
                return item.data ==
                    DefaultConstructible::default_data;
            }));
    }
    ...
    void testStaticAsserts() {
        ...
        static_assert(std::is_default_constructible_v<Deque<T1>>,
            "should have default constructor");
        static_assert(std::is_default_constructible_v<Deque<T2>>,
            "should have default constructor");
        static_assert(std::is_copy_constructible_v<Deque<T1>>,
            "should have copy constructor");
        static_assert(std::is_copy_constructible_v<Deque<T2>>,
            "should have copy constructor");
        ...
        static_assert(std::is_copy_assignable_v<Deque<T1>>,
            "should have assignment operator");
        static_assert(std::is_copy_assignable_v<Deque<T2>>,
            "should have assignment operator");
    }
    ...
    void testOperatorSubscript() {
        ...
        try {
            defaulted.at(static_cast<size_t>(-1));
        }
    ...
    void testStaticAssertsIterators() {
        ...
        CheckIter<decltype(std::declval<Deque<int>>().cbegin()), const int>
            const_iter;
        std::ignore = const_iter;

        static_assert(
            std::is_convertible_v<decltype(std::declval<Deque<int>>().begin()),
            decltype(std::declval<Deque<int>>().cbegin())>,
            "should be able to construct const iterator from non const iterator");
        static_assert(
            !std::is_convertible_v<decltype(std::declval<Deque<int>>().cbegin()),
    ...
    void testIteratorsArithmetic() {
        ...
        assert((empty.rbegin() + 0 == empty.rend()) &&
            (empty.rend() - 0 == empty.rbegin()));
        ...
        assert((d.begin() + d.size() == d.end()) &&
            (d.end() - d.size() == d.begin()));
    }
    ...
    void testPushAndPop() {
        ...
        assert(std::all_of(d.begin(), d.end(), [](const auto& item) {
            return item.data == 1;
            }));
